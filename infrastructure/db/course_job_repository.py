import json
from domain.models import CourseJob, Course
...
class CourseJobRepository:
    ...
    def save_job(self, job: CourseJob):
        with DatabaseConnectionManager() as cursor:
            ...
            final_course_json = json.dumps(job.final_course.__dict__) if hasattr(job.final_course, '__dict__') else (json.dumps(job.final_course) if job.final_course else None)
            ...
            if exists:
                ...
                cursor.execute("""
                    INSERT INTO course_jobs (job_id, id_usuario, prompt, status, course_outline, sections_with_candidates, final_course, error_message)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                """, (job.job_id, job.user_id, job.prompt, job.status, outline_json, candidates_json, final_course_json, job.error_message))

    def get_job(self, job_id: str) -> CourseJob:
        with DatabaseConnectionManager() as cursor:
            cursor.execute("SELECT * FROM course_jobs WHERE job_id = %s", (job_id,))
            row = cursor.fetchone()
            if not row:
                return None
